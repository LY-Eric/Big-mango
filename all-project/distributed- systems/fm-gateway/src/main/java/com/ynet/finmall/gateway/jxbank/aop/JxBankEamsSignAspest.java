 ...
 * 
 * 异步通知接口
 * 当前只提供给嘉兴银行 二类户系统 的 异步通知接口的请求验密 和 响应加密
 * 
 * @author liuxin
 *
 */
@Aspect
@Component
public class JxBankEamsSignAspest {

	private static Logger logger = LoggerFactory.getLogger(JxBankEamsSignAspest.class);
	
	@Pointcut("execution (* com.ynet.finmall.gateway.jxbank.controller.eams*.*(..))")
...
//        JSONObject jsonObj = JSONObject.parseObject(reqStr);
	...
	public static boolean checkSign(String resultString){
		...
		if(!checkSign){
			throw new JxBankEamsGatewayException(GatewayErrorConstants.JX_CHECK_SIGN);
		}
		return checkSign;
	}
